...
import { h, raw, escape } from './html.ts';
import { fmt } from '../lib/money.ts';
import { CATEGORIES } from '../lib/db.ts';

export function heroBlock(delta: number, label: string, caveat: string): string {
  const over = delta > 0;
...
export function monthBars(months: { m: string; expense: number }[]): string {
...
export function catRows(mix: { category: string; total: number }[], max?: number): string {
  const top = max ?? Math.max(...mix.map(c => c.total), 1);
  return mix.map(c => h`<div class="cat">
...
export function statusChips(chips: { cls: string; label: string }[]): string {
  return h`<div class="status">${raw(chips.map(c =>
    `<span class="chip"><span class="dot ${c.cls}"></span>${escape(c.label)}</span>`).join(''))}</div>`;
}

export function categoryChips(name: string, current?: string): string {
